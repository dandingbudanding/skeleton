...
void thinningIteration(cv::Mat& img, int iter)
{
    ...
    for (y = 1; y < img.rows-1; ++y) {
        ...
        for (x = 1; x < img.cols-1; ++x) {
			...
			/*int m1 = iter == 0 ? (*no * *ea * *so) : (*no * *ea * *we);//奇数次用2 4 6+4 6 8；偶数次用2 4 8+2 6 8
...
void thinning(const cv::Mat& src, cv::Mat& dst)
{
	...
	int count = 0;
    do {
        //thinningIteration(dst, 0);//根据情况调整，0：2 4 6+4 6 8；1：2 4 8+2 6 8
...
int main()
{
	cv::Mat src = cv::imread("f=50_r=8.8非球面球心2.bmp");
	if (!src.data)
		return -1;
	...
	std::cout << "thinning 操作用时：" << Te/1000 << "ms" << std::endl;
